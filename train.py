...
from dataset.dataset_CXR import NIH as data
...
from models import *
from utils.eval_utils import compute_metrics, compute_accuracy
...
def train():
    ...
    for epoch in range(options.epochs):
        ...
        for batch_id, (data, target, path_name) in enumerate(train_loader):
            global_step += 1
            ...
            if (batch_id + 1) % options.disp_freq == 0:
                ...
                #train_auc = compute_metrics(torch.cat(outputs), torch.cat(targets))
                train_acc = compute_accuracy(torch.cat(outputs), torch.cat(targets))
                log_string("epoch: {0}, step: {1}, train_loss: {2:.4f} train_acc: {3: .4f}"
...
def evaluate(**kwargs):
    ...
    with torch.no_grad():
        ...
        #test_auc = compute_metrics(torch.cat(outputs), torch.cat(targets))
        test_acc = compute_accuracy(torch.cat(outputs), torch.cat(targets))
